...
# Usage:
#   ./render_template.sh input.ndjson html/main.html output/keiste.html

# This script:
#   1. Generates a styled HTML table with inline SVGs/icons from NDJSON input.
#   2. Injects it into a template HTML file by replacing {template}.
#   3. Outputs the result as a final HTML file.
...
# --- Styling ---
BASE_STYLE="color: black; text-align: left; padding: 12px; font-weight: 400;"
ICON_STYLE="display: inline-block; width: 1em; height: 1em; vertical-align: middle;"
...
#    header=$(echo "$field" | sed -E 's/_/ /g' | awk '{print toupper(substr($0,1,1)) substr($0,2)}')
#    echo "    <th style=\"$BASE_STYLE\">$header</th>"
#  done
#  echo "  </tr>"
#} > "$TABLE_TMP"
...
awk -v table="$(cat "$TABLE_TMP")" '
  { gsub(/{template}/, table); print }
